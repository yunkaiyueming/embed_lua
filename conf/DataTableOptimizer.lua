...
local DatabaseRoot = "/Users/ray/Documents/Lua/embed_lua/conf"
-- local LuaRoot = Root.."/Assets/StreamingAssets/LuaRoot"
local LuaRoot = "/Users/ray/Documents/Lua/embed_lua/conf"
...
local function GetAllFileNamesAtPath( path )
	if true then return get_dir_files(path) end

	-- path, _ = path:gsub( "/", "\\" )
	local ret = {}
	print("get path = ",path)
	print(string.format( "ll \"%s\" ", path ))
	
	for dir in io.popen( string.format( "ll \"%s\" ", path ) ):lines() do
...
function get_dir_files(dir_path)
    ...
    while true do
		...
		if file_name~='DataTableOptimizer.lua' then
			-- local tmp_file_name = string.sub(file_name, 0, -5)
			table.insert(big_json, file_name)
		end
        start_pos = end_pos + 1
    end
...
local function LoadDataset( name )
	...
	local loader = function( name )
		...
		-- 	for dir in io.popen( string.format( "ls \"%s\" ", path ) ):lines() do
		-- 		local s, e, f = dir:find( ".+\\(.+%.lua)$" )
		-- 		if f then
		...
		-- 	os.execute( "pause" )
		-- end
		local fileName = "/Users/ray/Documents/Lua/embed_lua/conf/"..name
...
local function ExportDatabaseLocalText( tofile, newStringBank )
	...
	for _, v in ipairs( files ) do
		if not ExcludedFiles[ v ] then
			...
			print("t ret", t)
			if t then
				print("start ExportOptimizedDataset...")
...
ExportDatabaseLocalText( true, false )